...
static ERL_NIF_TERM set_helper(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
    struct gpio_priv *priv = enif_priv_data(env);
    ErlNifBinary helper_path;
    if (argc != 1 || !enif_inspect_binary(env, argv[0], &helper_path))
        return enif_make_badarg(env);

    char *s = malloc(helper_path.size + 1);
    memcpy(s, helper_path.data, helper_path.size);
    s[helper_path.size] = '\0';

    hal_set_helper_path(&priv->hal_priv, s);
...
static ErlNifFunc nif_funcs[] = {
    ...
    {"set_pull_mode", 2, set_pull_mode, ERL_NIF_DIRTY_JOB_IO_BOUND},
    {"pin", 1, pin_gpio, 0},
    {"info", 0, gpio_info, 0}
};
